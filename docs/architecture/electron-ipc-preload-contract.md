  ...
  interface Window {
    ask: {
      ...
      github: {
        ...
        startDeviceFlow(): Promise<AskResult<GitHubDeviceFlowStart>>;
        pollDeviceFlow(
          input: GitHubDeviceFlowPollRequest
        ): Promise<AskResult<GitHubDeviceFlowPoll>>;
      };
      ssh: {
...
| `ask:v1:github-device:start`   | `github.startDeviceFlow` | Start GitHub OAuth Device Flow fallback.                          | Requires configured OAuth client ID. Returns only session ID, user code, verification URL, expiry, and interval.         |
| `ask:v1:github-device:poll`    | `github.pollDeviceFlow`  | Poll Device Flow completion and resolve the GitHub username.      | Renderer provides only the opaque session ID. Access tokens stay in main memory and are discarded after username lookup. |
